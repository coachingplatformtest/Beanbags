-- Migration 006: Wipe ALL player props and reinsert exactly the 18 correct ones
...
BEGIN
  ...
  -- Wipe every player_prop (safe — win totals use category = 'season_win_total')
  DELETE FROM props WHERE category = 'player_prop';

  -- Also clean up any props from migration 002 that slipped through with wrong category
  DELETE FROM props WHERE game_id = fsu_nd_game;

  -- Reinsert exactly 18 correct props
  ...
  RAISE NOTICE 'Done — inserted 18 props for game %', fsu_nd_game;
END $$;
